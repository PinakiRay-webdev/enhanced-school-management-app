...
import { student_sidebar_nav_elements } from '../../../../../utils/utils';
...
const StudentSidebar = () => {
  ...
  return (
    <div className={`h-screen fixed z-40 top-0 ${sideBarStatus ? "w-[10vw]" : "w-[5vw]"} bg-white transition-all duration-150 ease-in-out`} >
      ...
      <main>
        {student_sidebar_nav_elements?.map((Element , id) =>(
            <NavLink to={`/student/${Element.link}`} className={({isActive}) => `${isActive ? "text-green-600" : "text-black"}`} >
