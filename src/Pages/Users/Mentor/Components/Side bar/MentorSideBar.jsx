...
import { mentor_sidebar_nav_elements, student_sidebar_nav_elements } from '../../../../../utils/utils';
...
const MentorSidebar = () => {
  ...
  return (
    <div className={`h-screen fixed z-40 top-0 ${sideBarStatus ? "w-[10vw]" : "w-[5vw]"} bg-white transition-all duration-150 ease-in-out`} >
      ...
      <main>
        {mentor_sidebar_nav_elements?.map((Element , id) =>(
            <NavLink to={`/mentor/${Element.link}`} className={({isActive}) => `${isActive ? "text-green-600" : "text-black"}`} >
